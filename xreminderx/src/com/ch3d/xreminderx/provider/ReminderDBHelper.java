...
public class ReminderDBHelper extends SQLiteOpenHelper
{
    ...
    {
        return getReadableDatabase().query(TABLE_NAME, null, selection, selectionArgs, null, null,
                sortOrder);
    }

    public Cursor queryAll()
    {
        return getReadableDatabase().query(TABLE_NAME, null, null, new String[] {}, null, null,
                "ts ASC");
    }

    public Cursor queryReminder(final long parseId)
    {
        return getReadableDatabase().query(TABLE_NAME, null, "_id = ?", new String[] {
                Long.toString(parseId)
        }, null,
                null, null);
    }
    ...
    public Cursor queryToday()
    {
        return getReadableDatabase().query(
                ...
                "ts > strftime(\"%s\",\"now\",\"-1 days\") * 1000" + " AND "
                        + "ts < strftime(\"%s\",\"now\",\"+1 days\") * 1000", null, null, null,
                "ts ASC");
    }
            ...
            final String[] selectionArgs)
    {
        ...
        return getWritableDatabase().update(TABLE_NAME, values, "_id = ?", new String[] {
                Long.toString(id)
        });
    }
