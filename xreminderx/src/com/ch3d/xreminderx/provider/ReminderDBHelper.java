...
public class ReminderDBHelper extends SQLiteOpenHelper {
	...
	private static final String SQL_CREATE_MAIN =
			"CREATE TABLE IF NOT EXISTS " + TABLE_NAME + "(" + "_id INTEGER PRIMARY KEY, " + "protocol INTEGER, " + "type INTEGER, "
					+ "ts TIMESTAMP NOT NULL DEFAULT current_timestamp, " + "alarm_ts BIGINT, " + "text VARCHAR(255), "
					+ "contactUri VARCHAR(255), " + "ongoing INTEGER, " + "silent INTEGER, " + "color INTEGER, " + "version INTEGER, "
					+ "pid VARCHAR(255), " + "account VARCHAR(255) " + ");";
	...
	public int deleteReminderForId(final long parseId) {
		return getWritableDatabase().delete(TABLE_NAME, "_id = ?", new String[]{Long.toString(parseId)});
	}
	...
	public void onUpgrade(final SQLiteDatabase db, final int oldVersion, final int newVersion) {
		...
		// db.execSQL("ALTER TABLE " + TABLE_NAME + " RENAME TO temp_" +
		// TABLE_NAME);
	...
	public Cursor queryReminder(final long parseId) {
		return getReadableDatabase().query(TABLE_NAME, null, "_id = ?", new String[]{Long.toString(parseId)}, null, null, null);
	}
