...
public class RemindersProvider extends ContentProvider {
	...
	public static Uri addReminder(final Context context, final ReminderEntry reminder, final boolean setUpAlarm) {
		final Uri result = context.getContentResolver().insert(RemindersProvider.REMINDERS_URI, ReminderUtils.getContentValues(reminder));
	...
	public Uri insert(final Uri uri, final ContentValues values) {
		final long id = mDbHelper.getWritableDatabase().insert(PATH_REMINDERS, StringUtils.EMPTY_STRING, values);
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs,
		...
		switch (sUriMatcher.match(uri)) {
			...
			case SEARCH:
				return mDbHelper.query(uri, projection, "text LIKE '%" + uri.getLastPathSegment() + "%'", selectionArgs, sortOrder);
	...
	public int update(final Uri uri, final ContentValues values, final String selection, final String[] selectionArgs) {
		...
		if ((rowsAffected > 0) && isGoogleApiConnected() && values.containsKey(RemindersContract.Columns.PID) && !StringUtils
				.isBlank(values.getAsString(RemindersContract.Columns.PID))) {
			addAccount(values);
