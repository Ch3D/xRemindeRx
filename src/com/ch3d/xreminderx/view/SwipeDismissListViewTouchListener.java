...
public class SwipeDismissListViewTouchListener implements View.OnTouchListener,
	...
	protected static final int				MSG_TRANSIENT_FALSE			= 0x100;
	 ...
	 */
	public SwipeDismissListViewTouchListener(final ListView listView,
			final DismissCallbacks callbacks) {
		final ViewConfiguration vc = ViewConfiguration.get(listView
				.getContext());
	...
	public boolean handleMessage(final Message msg) {
		switch (msg.what) {
			case MSG_TRANSIENT_FALSE:
				final View pendingView = (View) msg.obj;
				pendingView.setAlpha(1f);
				pendingView.setTranslationX(0);
				ViewCompat.setHasTransientState(pendingView, false);
				break;
	...
	public AbsListView.OnScrollListener makeScrollListener() {
		return new AbsListView.OnScrollListener() {
			@Override
			public void onScroll(final AbsListView absListView, final int i,
	...
	public boolean onTouch(final View view, final MotionEvent motionEvent) {
		...
		switch (motionEvent.getActionMasked()) {
			...
			case MotionEvent.ACTION_UP: {
				...
				final float absVelocityY = Math.abs(mVelocityTracker
						.getYVelocity());
	...
	private void performDismiss(final View dismissView,
		...
		for (final PendingDismissData pendingDismiss : mPendingDismisses) {
			ViewCompat.setHasTransientState(pendingDismiss.view, true);
		}
				...
				if (mDismissAnimationRefCount == 0) {
					...
					for (final PendingDismissData pendingDismiss : mPendingDismisses) {
						...
						mHandler.sendMessage(mHandler.obtainMessage(
								MSG_TRANSIENT_FALSE, pendingDismiss.view));
					}
