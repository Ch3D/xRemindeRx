...
public class FloatLabeledEditText extends LinearLayout {
	...
	private void setShowHint(final boolean show) {
		...
		if ((hintTextView.getVisibility() == VISIBLE) && !show) {
			...
			final ObjectAnimator move = ObjectAnimator.ofFloat(hintTextView, "translationY", 0, hintTextView.getHeight() / 8);
			...
			animation = new AnimatorSet();
			final ObjectAnimator move = ObjectAnimator.ofFloat(hintTextView, "translationY", hintTextView.getHeight() / 8, 0);
	...
	public interface KeyboardHideListener {
		public void onKeyboardHide(EditText view);
	}
